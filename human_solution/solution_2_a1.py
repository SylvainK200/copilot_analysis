...
def subset_sum2(list_num, wished_sum):
    ...
    nb_notes = length
    dp = [list([None] * (wished_sum + 1)) for i in range(nb_notes)]

    for sum_ in range(wished_sum + 1):
        for last_index in range(nb_notes):
            if sum_ == 0:
...
def main():
    ...
    for icase in range(nb):
        nb_notes, asked_money = list(map(int, data.readline().split()))  # int
        notes = []
